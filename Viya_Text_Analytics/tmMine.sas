...
caslib stopl datasource=(srctype="path") path="/home/sasdemo/StopLists" sessref=mySession subdirs;

caslib _all_ assign;


data casuser.reviews;                                   /* 2*/
...
run;
                                                      ...
                                                      /* 3*/
proc cas;                                            
   loadtable caslib="stopl" path="en_stoplist.sashdat"; 
   run;
quit;
...
proc cas;                                             /* 4*/
   ...
   param
   docId="did"
   documents={caslib="casuser" name="reviews"}
   text="text"
   nounGroups= False
   tagging = True
   stemming= True
