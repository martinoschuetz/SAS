...
/* modifying and preparing data prior to modeling. It uses BANK_RAW     */
/* dataset as input and produces _PREPPED dataset. The _PREPPED dataset */
/* is not used in subsequent examples.                                  */
/*                                                                      */
/* The steps include:                                                   */
/*                                                                      */
...
%let sasdata          = locallib.bank_raw;                     
%let casdata          = mycaslib.bank_raw;

/* Specify the data set inputs and target */
%let class_inputs    = cat_input1 cat_input2;
%let interval_inputs = rfm1-rfm12; 
%let cluster_inputs  = r_demog_inc r_demog_homeval demog_age;
%let target          = b_tgt;
...
%if not %sysfunc(exist(&casdata)) %then %do;
  proc casutil;
    load data=&sasdata casout="bank_raw";
  run;
%end;
...
proc varimpute data=&casdata;
  input r_demog_homeval rfm3 r_demog_inc /ctech=mean;
  output out=mycaslib._prepped copyvars=(_ALL_);
...
proc varreduce data=mycaslib._prepped technique=discriminantanalysis;  
...
/************************************************************************/
proc kclus data=mycaslib._prepped standardize=std distance=euclidean maxclusters=6;
...
/* input variables                                                      */     
/************************************************************************/
proc pca data=mycaslib._prepped   plots=(scree);
  var rfm1 rfm2 rfm4-rfm12 IM_rfm3;
run;
