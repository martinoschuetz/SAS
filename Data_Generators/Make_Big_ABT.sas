%macro syndata(
	...
	%put NOTE: approx size is ( &reps * &NR * &NG * (&NNsize+&NCsize) ) / (1024 * 1024 * 1024);
	%put NOTE: approx size is %eval( ( &reps * &NR * &NG * (&NNsize+&NCsize) ) / (1024 * 1024 * 1024) ) GB;
	%let norm_mean = 72;
	%let norm_std  = 12;
	%let x_mean = 0.3;
	%let x_std  = 0.2;
			...
			data &lib..&data(&options) &slash;
				...
				do r=1 to rows;
					do i = 1 to &Nnorm;
						...
						else norm[i] = RAND('NORMAL') * &norm_std + &norm_mean;
					end;
...
%syndata(lib=mycas, data=synth
	, nnorm=20, nunif=20, ng=1000, nr=100, reps=5
	, ctlopt=%str(promote=yes)
	, options=%str(promote=yes)
	, slash=%str(/threads=2)
	, ctlonly=N
	);